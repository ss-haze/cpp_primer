...
class QueryResult
{
  using QueryIter = set<StrBlob::size_type>::iterator;

  friend std::ostream &print(std::ostream &os, const QueryResult &q);

  QueryIter begin() const { return set_ptr->begin(); }
  QueryIter end() const { return set_ptr->end(); }
  shared_ptr<StrBlob> get_file() const { return std::make_shared<StrBlob>(line_ptr); }
